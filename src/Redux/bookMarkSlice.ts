  ...
  reducers: {
    ...
    removeBookMark: (state: IState, action: PayloadAction<IBookMark>) => {
      ...
      if (contentType === "movie") {
        ...
        return {
          movieBookMarks: movieBookMarks,
          seriesBookMarks: seriesBookMarks.filter(
            (bookMark) => bookMark.id !== action.payload.id
          ),
        };
      }
...
export const selectMovieBookMarks = (state: RootState) => {
  return state.bookMark.movieBookMarks.map(
    (movieBookMark) => movieBookMark["bookMarkInfo"]
  );
};
export const selectSeriesBookMarks = (state: RootState) => {
  return state.bookMark.seriesBookMarks.map(
    (seiresBookMark) => seiresBookMark["bookMarkInfo"]
  );
};
